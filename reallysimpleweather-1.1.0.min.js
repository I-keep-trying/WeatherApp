var reallySimpleWeather = {
	...
	weather: function (a) {
		...
		if ("" !== a.wunderkey) {
			...
			d = "";
			d = /^(\-?\d+(\.\d+)?),\s*(\-?\d+(\.\d+)?)$/.test(a.location) ? "(" + a.location + ")" : a.location,
			c += "select * from weather.forecast where woeid in (select woeid from geo.places(1) where text='" + d + "') and u='" + a.unit + "'"
		} else {
			if ("" === a.woeid)
				return a.error("Could not retrieve weather due to an invalid location."), !1;
			c += "select * from weather.forecast where woeid=" + a.woeid + " and u='" + a.unit + "'"
		}
		var e = new XMLHttpRequest;
		return e.open("GET", encodeURI(c)),
		e.send(null),
		e.onreadystatechange = function () {
			...
			if (e.readyState === b) {
				...
				if (d.response) {
					...
					g.title = "Conditions for " + f.current_observation.display_location.full + " at " + f.current_observation.observation_time,
					g.temp = f.current_observation.temp_f,
					g.temp_f = f.current_observation.temp_f,
					g.temp_c = f.current_observation.temp_c,
					g.feelslike_f = f.current_observation.feelslike_f,
					g.feelslike_c = f.current_observation.feelslike_c,
					g.visibility_mi = f.current_observation.visibility_mi,
					g.visibility_km = f.current_observation.visibility_km,
					g.temp_string = f.current_observation.temperature_string,
					g.currently = f.current_observation.weather,
					g.high = f.almanac.temp_high.normal.F,
					g.high_c = f.almanac.temp_high.normal.C,
					g.low = f.almanac.temp_low.normal.F,
					g.low_c = f.almanac.temp_low.normal.C,
					g.text = g.currently,
					g.humidity = f.current_observation.relative_humidity,
					g.pressure = f.current_observation.pressure_mb,
					g.sunrise = f.sun_phase.sunrise.hour + ":" + f.sun_phase.sunrise.minute,
					g.sunset = f.sun_phase.sunset.hour + ":" + f.sun_phase.sunset.minute,
					g.city = f.current_observation.display_location.city,
					g.state = f.current_observation.display_location.state,
					g.state_name = f.current_observation.display_location.state_name,
					g.country = f.current_observation.display_location.country,
					g.full = f.current_observation.display_location.full,
					g.zip = f.current_observation.display_location.zip,
					g.latitude = f.current_observation.display_location.latitude,
					g.longitude = f.current_observation.display_location.longitude,
					g.elevation = f.current_observation.display_location.elevation,
					g.updated = f.current_observation.observation_time,
					g.link = f.current_observation.forecast_url,
					g.wind = {
					...
					g.forecast = [];
					for (var j = 0; j < f.forecast.simpleforecast.forecastday.length; j++)
						h = f.forecast.simpleforecast.forecastday[j], g.forecast.push(h);
					a.success(g)
				} else {
					var h,
					...
					"f" === a.unit ? g.alt.unit = "c" : g.alt.unit = "f",
					g.forecast = [];
					for (var j = 0; j < f.item.forecast.length; j++)
						h = f.item.forecast[j], h.alt = {
